...
class SimilarityProcessor:
    ...
    def process_lists(self):
        ...
        self.selected_elements = self.select_dissimilar_elements(flat_list)
...
class Trader:
    ...
    def __init__(self, sound_path):
        ...
        self.refreshtoken = 'eyJzaWduX3RpbWUiOiIyMDI1LTAzLTIwIDExOjUwOjA2In0=..AD71833F1902958AD852E7C76B599F709CA2C344269831932B50EA87055EAC9A'
            ...
            for i in range(self.times):
                ...
                if processor.real_prices_g is not None and self.real_prices:
                    ...
                    if processor_m.state:
                        ...
                        print("排序后的元素:", self.sorted_selected_elements)
    ...
    def sell_logic(self):
        ...
        #             self.close_prices.append(main_data['table']['ths_close_price_stock'])
        #         else:
        #             pass
        while self.buy_state_fina is not True:
            self.close_prices.clear()
            self.close_prices_15min.clear()
            self.ratio_list.clear()
            self.close_prices_15min_new.clear()
            self.close_prices_15min_new_.clear()
            self.codes_list_confirm.clear()
            self.end_time = time.time()
            self.buy_state = False
            self.buy_state_set.clear()
            # threading.Thread(target=self.high_frequency, args=[self.codes_list]).start()
            self.high_frequency(self.codes_list)
            print('-------------------------')
            ...
            for num1, num2 in zip(self.extract_last_elements(lower_band), self.real_prices_g):
                ...
                if num2 != 0:
                    ...
                    print(f'{self.codes_list[temp]}：{num2}对应的ratio值为：', self.ratio)
                else:
                    # 避免除零错误
                ...
                if self.ratio < self.ratio_threshold:
                    ...
                    print(self.ratio, self.codes_list_confirm)
...
class StockSearchApp:
    def __init__(self, root):
        ...
        self.stock_entry = ttk.Entry(left_frame, width=20, font=entry_font)
        self.stock_entry.grid(row=0, column=1, pady=5)
        ttk.Button(left_frame, text="添加", command=self.add_stock_code, style="Apple.TButton").grid(row=0, column=2, pady=5, padx=5)

        # 令牌填写框
        ttk.Label(left_frame, text="输入令牌:", font=label_font).grid(row=1, column=0, pady=5, sticky="w")
        self.token_entry = ttk.Entry(left_frame, width=20, font=entry_font)
        self.token_entry.insert(0, "221381")
        self.token_entry.grid(row=1, column=1, pady=5)
        ttk.Button(left_frame, text="设置", command=self.set_token, style="Apple.TButton").grid(row=1, column=2, pady=5, padx=5)

        # 延时时间输入框
        ttk.Label(left_frame, text="延时时间 (秒):", font=label_font).grid(row=2, column=0, pady=5, sticky="w")
        self.delay_entry = ttk.Entry(left_frame, width=20, font=entry_font)
        self.delay_entry.insert(0, "30")
        self.delay_entry.grid(row=2, column=1, pady=5)
        ttk.Button(left_frame, text="设置", command=self.set_delay_time, style="Apple.TButton").grid(row=2, column=2, pady=5, padx=5)

        # 比例阈值输入框
        ttk.Label(left_frame, text="比例阈值:", font=label_font).grid(row=3, column=0, pady=5, sticky="w")
        self.ratio_entry = ttk.Entry(left_frame, width=20, font=entry_font)
        self.ratio_entry.insert(0, "0.005")
        self.ratio_entry.grid(row=3, column=1, pady=5)
        ttk.Button(left_frame, text="设置", command=self.set_ratio_threshold, style="Apple.TButton").grid(row=3, column=2, pady=5, padx=5)

        # 数据展示面板
        self.data_display = tk.Text(left_frame, width=50, height=15, font=entry_font)
        ...
        # 按钮
        ttk.Button(right_frame, text="删除选中", command=self.delete_selected_code, style="Apple.TButton").grid(row=5, column=0, pady=5)
        ttk.Button(right_frame, text="清空全部", command=self.clear_all_codes, style="Apple.TButton").grid(row=6, column=0, pady=5)
        ttk.Button(right_frame, text="执行买入逻辑", command=self.execute_sell_logic, style="Apple.TButton").grid(row=7, column=0, pady=5)
        ttk.Button(right_frame, text="退出", command=self.root.destroy, style="Apple.TButton").grid(row=8, column=0, pady=5)
    ...
    def update_display(self):
        # 更新 Trader 数据
        # 构建要显示的数据
        data = f"股票代号: {self.trader.codes_list:.2f}\n"
        data += f"实时价格: {self.trader.real_prices}\n"
        data += f"实时下布林线: {self.trader.calculate_bollinger_bands():.2f}"
    ...
    def save_stock_codes(self):
        with open('stock_codes.json', 'w') as f:
    ...
    def load_stock_codes(self):
        if os.path.exists('stock_codes.json'):
            with open('stock_codes.json', 'r') as f:
                codes = json.load(f)
                for code in codes:
